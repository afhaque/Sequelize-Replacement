...
**Bonus** After creating a few Authors with Posts, try using POSTMAN to perform a get request to `localhost:8080/api/authors` to see what our Author-Post joined data looks like. Try the same for `localhost:8080/api/posts`. This is what's known as a left-outer-join.
