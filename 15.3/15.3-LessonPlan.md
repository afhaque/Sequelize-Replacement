...
* Stress to students that today will be all back-end coding. The exercises are all built in a way where they don't need to modify any client side code in order to better focus on Sequelize.
...
**Note**: We still have some code to add in the next exercise to get this app fully functioning, concentrate on creating the Post model for now.
...
* Explain to the class that what we just went over what can be considered both a belongsTo association, as well as a hasMany association in Sequelize. This is one of the most common types of associations.
...
**Bonus**: If you complete the exercise, navigate to the author.js file and add a hasMany association from the Author to Post. A perfect example can be found here: http://docs.sequelizejs.com/en/1.7.0/articles/express/#modelsuserjs
