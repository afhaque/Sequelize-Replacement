...
4) Add a Sequelize method inside the GET route which finds all of the todos and returns them to the user as JSON.

5) Add a Sequelize method to the POST route to save a new todo to the database using the data sent to the server in req.body.
...
7) Hint: Here's a link to Sequelize's documentation for it's models API `http://docs.sequelizejs.com/en/latest/api/model/`
...
INSTRUCTIONS:
 ...
 7) **HINT**: you will need to pass in an options object with a "where" attribute into both methods in order to filter these queries to target the Todos we want.
 http://docs.sequelizejs.com/en/latest/docs/querying/#where

 9) **HINT**: The method to delete a record may not be called what you think it's called.
 http://docs.sequelizejs.com/en/latest/api/model/#destroyoptions-promiseinteger

 10) Navigate to localhost:8080. If you can update and delete todos without errors, you were successful.
...
* Explain to students that at first the Sequelize documentation can be a little difficult to navigate, but all of the model methods with examples can be found here. http://docs.sequelizejs.com/en/latest/api/model/
...
7) Navigate to localhost:8080 to test that this worked. Try to save a Todo without any text in it, and then try and save a Todo with over 140 characters. If you see errors in your console that have to do with validation, you were successful.

8) **Big Hint**: Sequelize documentation on validations can be found here http://docs.sequelizejs.com/en/latest/docs/models-definition/#validations
...
* Open the `todo.js` file inside of Answer any remaining questions and slack out [`4-Sequelize-Validations-Solved`](Activities/4-Sequelize-Validations/4-Sequelize-Validations-Solved) and show students how we added validations and flags to the Todo model. Go through each new item and have students try and explain to you what they believe each piece of new code does.
...
* As students who've completed the activity have probably noticed, when our validations fail, no records are inserted into our table, but an error is thrown. These cause our entire node application to crash. If this happened while our app was deployed to Heroku, the entire site would go down.

* Open the `api-routes.js` file and show the class how we've also added a .catch function at the end of our Sequelize queries. While not necessary, this is great to do in a production setting since we can "catch" the thrown error and do something with it, such as logging it out to the user or writing it to a text file, rather than letting it crash out app. 

* Inform the class that we've just covered a lot of ground today. We've learned everything and more needed to learn to complete this week's homework assignment. If they feel they haven't caught onto everything, encourage them to check out the solution files and browse through the Sequelize documentation, or better yet, try and build something simple with Sequelize. Of course, they can also talk to you or a TA between classes for one-on-one advice.
