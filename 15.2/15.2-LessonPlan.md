...
* Inform the class that they will be implmenting functionality to retrieve todos from the database using the Sequelize "findAll" and "create" methods.
...
If you get stuck or finish early, try and see if you and your partnet can make sense of Sequelize's docs for the findAll and create methods
...
INSTRUCTIONS:
 ...
 4) Inside the `api-routes.js` file, look for the DELETE route and add a Sequelize method to delete the todo with the id available to us in `req.params.id`. (db.Todo.destroy)

 5) Inside of the same file, look for the PUT route and add a Sequelize method to update a todo with the new todo data available to us inside req.body. (db.Todo.update)
...
* Inform the class that we've just covered a lot of ground today. We've learned everything and more needed to complete this week's homework assignment. If they feel they haven't caught onto everything, encourage them to check out the solution files and browse through the Sequelize documentation, or better yet, try and build something simple with Sequelize. Of course, they can also talk to you or a TA between classes for one-on-one advice.
