...
module.exports = function(app) {
  ...
  // ------------------------------------------------------------------------------------------------------
  // By saying app.all (all routes will pass through here.
  // If they meet the requirement for a token then they are "next"ed to the next route option).
  ...
  app.all("*", function(req, res, next) {
    ...
    jwt.verify(token, app.get("jwtSecret"), function(err, decoded) {
      if (err) {
        ...
        res.json({
          success: false,
          message: "access denied. Bro. Did you even send me a token?"
        });
      }
      else {
